...
// StandardOps lists the bulk of the operations that a backend must support.
// Only scalars (0-dimensional arrays) are broadcasted.
type StandardOps interface {
	...
	// So the number of axes of the result is one less than the number of axes of x.
	//
	...
	ArgMinMax(x Value, axis int, outputDType dtype.DType, isMin bool) (Value, error)
	...
	//
	// It returns the normalized array, the batchMean, and the batchVariance.
	//
	// Based on the paper "Batch Normalization: Accelerating Deep Network Training by Reducing
	// Internal Covariate Shift" (Sergey Ioffe, Christian Szegedy), https://arxiv.org/abs/1502.03167.
	...
	// the axes are not changed, simply the dtype is changed.
	//
	// If targetDType.Size() > x.DType().Size(), it requires x last axis to have a length of
	// targetDType.Size() / x.DType().Size(), and the returned array type will trim the last axis.
	//
	// If targetDType.Size() < x.DType().Size(), the returned array type will have an extra axis in the end, with length
	...
	Bitcast(x Value, targetDType dtype.DType) (Value, error)
	...
	// BroadcastInDim broadcasts x to an output with the given array type.
	// broadcastAxes has an output axes value for each x axes (len(broadcastAxes) == x.NumAxes()).
	// The i-th axis of x is mapped to the broadcastAxes[i]-th axis of the output.
	...
	// output axis it's broadcasting into.
	// For example, say operand `x = (s32)[2]{1, 2}`; outputArrayType = `(s32)[2,2]`:
	...
	//     {{1 , 1},
	//     {2 , 2}}
	BroadcastInDim(x Value, outputArrayType atype.ArrayType, broadcastAxes []int) (Value, error)
	...
	//
	// The values max and min can either be a scalar or have the same axes as x.
	...
	// The axes of `real` or `imaginary` must be the same, or one must be a scalar, in which case
	// the value is broadcast to every other value.
	Complex(lhs, rhs Value) (Value, error)

	// Concatenate operands on the given axis.
	//
	// All axes that are not being concatenated must match axis lengths.
	...
	ConvertDType(x Value, dtype dtype.DType) (Value, error)
	...
	// The exact semantics of this operation depend on the number of axes of the operands:
	// | Input | Output | Semantics |
	// | vector [n] dot vector [n] | scalar | vector dot product |
	// | matrix [m x k] dot vector [k] | vector [m]	matrix-vector multiplication |
	// | matrix [m x k] dot matrix [k x n] | matrix [m x n] | matrix-matrix multiplication |
	// The operation performs sum of products over the second axis of x0 (or the first if it has 1 axis) and
	// the first axis of x1.
	// These are the "contracted" axes.
	// The contracted axes of x0 and x1 must have the same length.
	...
	//
	//   - Just aligned (batch axes), so the output has the same axes as the inputs. The axis lengths
	...
	//     those axes.
	//
	// It follows that the resulting axis order starts with the batch axes, then the 'lhs'
	// non-contracting/non-batch axes, and finally the 'rhs' non-contracting/non-batch axes.
	...
	//
	// - operand: array from where to take the slice.
	// - startIndices: scalar arrays, one per axis of operand: len(startIndices) == operand.NumAxes().
	// - sliceSizes: static values and fixed to keep the array type of the output static.
	//
	// The startIndices are adjusted as follows:
	//
	//	adjustedStartIndices[i] = clamp(0, StartIndices[i], operand.AxisLength(i) - sliceSizes[i])
	//
	...
	// - sliceSizes: static values and fixed to keep the array type of the output static.
	//
	// It returns a value with the same axes as the operand, with the values updated.
	//
	// The startIndices are adjusted as follows:
	//
	//	adjustedStartIndices[i] = clamp(0, StartIndices[i], operand.AxisLength(i) - update.AxisLength(i))
	DynamicUpdateSlice(operand, update Value, startIndices []Value) (Value, error)

	// Equal performs element-wise equality check, returns boolean results with the same axes as input.
	...
	//
	// Its output array type will be composed of 2 parts:
	//
	...
	//   - "Offset axes": these are axes that come from the operand, the lengths given by sliceSizes.
	...
	//
	// So in general output.NumAxes() = startIndices.NumAxes() - 1 + len(offsetAxes).
	//
	// (*) One exception is if indexVectorAxis == startIndices.NumAxes(), in which case we assume there is an
	// extra implicit axis in startIndices of length 1, in which case output.NumAxes() = startIndices.NumAxes() + len(offsetAxes).
	//
	// Arguments:
	...
	//     All other axes are "batch axes" and they will have equivalent axes (same lengths) in the output.
	...
	//     It is typically the last axis of startIndices, so startIndices.NumAxes()-1.
	//     There is a special case where indexVectorAxis == startIndices.NumAxes() in which case we assume there is an
	//     extra virtual axis in startIndices of length 1, in which case output.NumAxes() = startIndices.NumAxes() + len(offsetAxes).
	...
	//     The len(offsetOutputAxes) must match the length of indexVectorAxis (== startIndices.AxisLength(indexVectorAxis)).
	...
	//     One must have operand.NumAxes() == len(collapsedSliceAxes) + len(offsetAxes).
	...
	//     Also, one must have operand.NumAxes() == len(collapsedSliceAxes) + len(offsetOutputAxes).
	...
	//     Notice len(startIndexMap) must match startIndices.AxisLength(indexVectorAxis).
	//     Also, len(startIndexMap) == len(offsetOutputAxes) -- offsetOutputAxes maps the same axes in the output.
	//     E.g.: if startIndices.ArrayType=(2, 3), indexVectorAxis=1, and operand.NumAxes()=4 and startIndexMap=[]int{0, 1, 2},
	//     this means each row of the startIndices will point to the first 3 axes (0,1 and 2) in the operand.
	//     In many cases this is [0, 1, 2, ..., operand.NumAxes()-1], that is, each "index vector" fully defines
	//     an element on the operand. In some this is only a prefix of the operand's number of axes.
	//     For those axes in the operand not explicitly set (so if len(startIndexMap) < operand.NumAxes()), the corresponding
	//     axis start index is considered to be 0, and one sets the sliceSizes to take the slice one wants (typically the
	//     full slice).
	//   - sliceSizes: a length for each operand's axis, so len(sliceSize) = operand.NumAxes().
	...
	//     in some platforms.
	//
	// Out-of-bound (and negative) indices <i> are adjusted with max(min(<i>, axisLength-1), 0), meaning they
	...
	// GreaterOrEqual performs element-wise comparison, returns boolean results with the same axes as input.
	...
	// GreaterThan performs element-wise comparison, returns boolean results with the same axes as input.
	...
	// Iota creates a constant of the given array type with increasing numbers (starting from 0)
	// on the given axis. So Iota([2,2], 1) returns [[0 1][0 1]], while Iota([2,2], 0)
	// returns [[0 0][1 1]].
	Iota(arrayType atype.ArrayType, iotaAxis int) (Value, error)
	...
	// It returns the same axes as the input, but with boolean values where each element is true if and only if
	...
	// LessOrEqual performs element-wise comparison, returns boolean results with the same axes as input.
	...
	// LessThan performs element-wise comparison, returns boolean results with the same axes as input.
	...
	// NotEqual performs element-wise inequality check, returns boolean results with the same axes as input.
	...
	// There must be at most `operand.NumAxes()` axesConfig values. Missing PadAxis are assumed to be zeros,
	...
	// Reshape reshapes x to the new axis lengths.
	...
	// Reverse returns x with the values for the given axes reversed, that is,
	// the value indexed at `i` will be swapped with the value at indexed `(axis_length - 1 - i)`.
	// The axes remain the same.
	Reverse(x Value, axes ...int) (Value, error)

	// RNGBitGenerator generates an array of the given array type filled with random bits.
	//
	...
	RNGBitGenerator(state Value, arrayType atype.ArrayType) (newState Value, values Value, err error)
	...
	// The subarray has the same number of axes as the input and contains the values inside a bounding box within the input array
	// where the axis lengths and indices of the bounding box are given as arguments to the slice operation.
	// The strides set the input stride of the slice in each axis and must be >= 1.
	// It is optional, and if missing, it is assumed to be 1 for every axis.
	// Examples:
	...
	// The output will have: output.AxisLength(ii) = x.AxisLength(permutations[i]).
	...
	//
	// If either condition, onTrue or onFalse is a scalar, it will be broadcasted to the axes of the other operands.
...
// ConvolveAxesConfig defines the interpretation of the input/kernel/output array axes.
// There must be the same number of spatial axes for each of the 3 arrays.
