...
func TestShape(t *testing.T) {
	...
	shape1 := Make(dtype.Float32, 4, 3, 2)
...
func TestDim(t *testing.T) {
	shape := Make(dtype.Float32, 4, 3, 2)
...
func TestFromAnyValue(t *testing.T) {
	...
	require.NotPanics(t, func() { shape.Assert(dtype.Int32, 3) })

	shape, err = FromAnyValue([][][]complex64{{{1, 2, -3}, {3, 4 + 2i, -7 - 1i}}})
	require.NoError(t, err)
	require.NotPanics(t, func() { shape.Assert(dtype.Complex64, 1, 2, 3) })
